...
<body>
	...
	<div id="fh5co-page">
		...
		<div id="fh5co-main">
			...
			<tbody>
				<%
					List workers = (List) request.getAttribute("workerlist");
					int listsize = workers.size();
					int i = 0;
					while (i < listsize) {
						String url = "./findworker_Find.action?workerId=";
						url += ((TWorkerInfo) workers.get(i)).getWorkerId();
						String url1 = "./findworker_DelInfo.action?workerId=";
						url1 += ((TWorkerInfo) workers.get(i)).getWorkerId();
				%>
				<tr>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkerId()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkerName()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getTypeOfWork()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkStatus()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkProgress()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkBeltline()%></td>
					<td><%=((TWorkerInfo) workers.get(i)).getWorkStation()%></td>
					<td><a href=<%=url%>>修改 </a> <a href=<%=url1%>>删除 </a></td>
				</tr>
				<%
					i++;
					}
